...
export default function EmailVerificationScreen() {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const result = await AuthService.resetPassword(data.email); // Use AuthService
      if (result.success) {
        setEmailSent(true);
        ToastAndroid.show("Reset email sent!", ToastAndroid.SHORT);
        navigation.navigate("ChangePassword", { email: data.email, isReset: true });
      } else {
        let errorMessage = result.error || "Failed to send reset email. Please try again.";
        ToastAndroid.show(errorMessage, ToastAndroid.LONG);
      }
    } catch (error) {
      ToastAndroid.show("Failed to send reset email. Please try again.", ToastAndroid.LONG);
    } finally {
      setLoading(false);
    }
  };
